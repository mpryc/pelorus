...
def _remove_sha256_from_cache(sha_256: str) -> None:
    with image_label_cache_lock:
        logging.debug(f"Removing SHA256 from the cache: {sha_256} ")
...
def get_labels_from_image(sha_256: str, image_uri: str):
...
class ContainerImageCommitCollector(AbstractCommitCollector):
    ...
    def generate_metrics(self) -> Iterable[CommitMetric]:
        ...
        for _, pod in replica_pods_dict.items():

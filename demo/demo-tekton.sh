...
Help()
{
   ...
   echo "i     If set then no human interaction is required for subsequent deployments"
...
# Get the options
while getopts ":hg:b:r:n:t:c:a:i" option; do
   case $option in
      h) # display Help
         Help
         exit;;
...
oc process -f "$tekton_setup_dir/04-service-account_template.yaml" -p PROJECT_NAMESPACE="${app_namespace}" | oc create -f -

echo "5. Setting up build and deployment information"
oc process -f "$tekton_setup_dir/05-build-and-deploy.yaml" -p NAMESPACE="${app_namespace}" -p APPLICATION_NAME="${app_name}" > /tmp/05-build-and-deploy.yaml.out 2>/tmp/05-build-and-deploy.yaml.err
